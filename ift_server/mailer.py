"""Standalone copy of root mailer.py for IFT Linux deploy.

Keep in sync with ../mailer.py HTML send path when that changes.
Plain-email helpers are omitted here (IFT uses send_news_email only).
"""
...
def resolve_email_header_path(custom_path=None) -> Path:
    ...
    if not path.is_file():
        raise SystemExit(
            ...
            "или задайте EMAIL_HEADER_IMAGE в .env."
        )
...
def require_llm_columns_for_send(df: pd.DataFrame, excel_path=None) -> None:
    ...
    if missing:
        ...
        raise SystemExit(
            f"Ошибка: во входящем Excel нет LLM-колонок{path_hint}.\n"
            f"Отсутствуют: {missing_text}.\n"
            "Файл должен быть после enrich на стороне парсера."
        )
